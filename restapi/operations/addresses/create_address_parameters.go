package addresses

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"io"
	"net/http"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/runtime"
	"github.com/go-openapi/runtime/middleware"

	"github.com/EnMasseProject/enmasse-rest/models"
)

// NewCreateAddressParams creates a new CreateAddressParams object
// with the default values initialized.
func NewCreateAddressParams() CreateAddressParams {
	var ()
	return CreateAddressParams{}
}

// CreateAddressParams contains all the bound params for the create address operation
// typically these are obtained from a http.Request
//
// swagger:parameters createAddress
type CreateAddressParams struct {

	// HTTP Request Object
	HTTPRequest *http.Request

	/*AddressConfig to create
	  Required: true
	  In: body
	*/
	AddressConfigMap models.AddressConfigMap
}

// BindRequest both binds and validates a request, it assumes that complex things implement a Validatable(strfmt.Registry) error interface
// for simple values it will use straight method calls
func (o *CreateAddressParams) BindRequest(r *http.Request, route *middleware.MatchedRoute) error {
	var res []error
	o.HTTPRequest = r

	if runtime.HasBody(r) {
		defer r.Body.Close()
		var body models.AddressConfigMap
		if err := route.Consumer.Consume(r.Body, &body); err != nil {
			if err == io.EOF {
				res = append(res, errors.Required("addressConfigMap", "body"))
			} else {
				res = append(res, errors.NewParseError("addressConfigMap", "body", "", err))
			}

		} else {
			if err := body.Validate(route.Formats); err != nil {
				res = append(res, err)
			}

			if len(res) == 0 {
				o.AddressConfigMap = body
			}
		}

	} else {
		res = append(res, errors.Required("addressConfigMap", "body"))
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}
